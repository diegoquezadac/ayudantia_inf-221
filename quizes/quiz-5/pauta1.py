...
def monte_carlo_rule(f,a,b,n):
    mu, sigma = 0.93, 0.3
    points = np.random.lognormal(mu, sigma, 10000)
    suma_imagenes = 0
    for x in points:
        suma_imagenes = suma_imagenes + f(x)
    return suma_imagenes / n


f,a,b,n = (lambda x: 1/(x**21 - 21 * np.log(np.sqrt(x)) ) , 0.1, 1000, 50)
...
monte_carlo = monte_carlo_rule(f, a, b, n)


print("Rectangle rule relative error: " + str(abs(real_value - rectangle)  / real_value))
print("Trapezoidal rule relative error: " + str(abs(real_value - trapezoidal) / real_value))
print("Simpson rule relative error: " + str(abs(real_value - simpson) / real_value))
print("Monte carlo rule relative error: " + str(abs(real_value - monte_carlo) / real_value))